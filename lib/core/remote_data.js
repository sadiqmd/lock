  ...
  m = (0, _sync2.default)(m, 'sso', {
    ...
    errorFn: function errorFn(m, error) {
      if (error.error === 'consent_required') {
        ...
        l.warn(
          m,
          'There was an error fetching the SSO data. This could simply mean that there was a problem with the network. But, if a "Origin" error has been logged before this warning, please add "' +
            origin +
            '" to the "Allowed Web Origins" list in the Auth0 dashboard: ' +
            appSettingsUrl
        );
      }
    }
