...
function setField(m, field, value) {
  ...
  for (
    var _len = arguments.length, args = Array(_len > 4 ? _len - 4 : 0), _key = 4;
    _key < _len;
    _key++
  ) {
...
function validate(validator, value) {
  ...
  return validation &&
    (typeof validation === 'undefined' ? 'undefined' : _typeof(validation)) === 'object'
    ? (0, _immutable.Map)({ valid: validation.valid, invalidHint: validation.hint })
    : (0, _immutable.Map)({ valid: !!validation });
}
...
function registerOptionField(m, field, options, initialValue) {
  ...
  if (!valid || !options.size) {
    var stopError = new Error(
      'The options provided for the "' +
        field +
        '" field are invalid, they must have the following format: {label: "non-empty string", value: "non-empty string"} and there has to be at least one option.'
    );
...
function getField(m, field) {
  var notFound =
    arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : new _immutable.Map({});
...
function renderOptionSelection(m) {
  ...
  return isSelecting(m)
    ? _react2.default.createElement(_option_selection_pane2.default, {
        model: m,
        name: name,
