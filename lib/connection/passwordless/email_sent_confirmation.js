...
var Resend = (function(_React$Component2) {
  ...
  Resend.prototype.render = function render() {
    ...
    var resendLink =
      ...
      _react2.default.createElement(ResendLink, {
        onClick: this.handleClick.bind(this),
        label: m.resendFailed(lock) ? labels.retry : labels.resend
      });

    var resendingLabel =
      m.resendOngoing(lock) &&
      _react2.default.createElement('a', { className: 'auth0-lock-resend-link' }, labels.resending);

    var resendSuccessLabel =
      m.resendSuccess(lock) &&
      _react2.default.createElement('span', { className: 'auth0-lock-sent-label' }, labels.sent);

    var resendFailedLabel =
      m.resendFailed(lock) &&
...
function renderEmailSentConfirmation(lock) {
  ...
  return m.passwordlessStarted(lock)
    ? _react2.default.createElement(EmailSentConfirmation, props)
    : null;
}
