...
function assertMaybeBoolean(opts, name) {
  ...
  if (!valid)
    l.warn(opts, 'The `' + name + '` option will be ignored, because it is not a booelan.');
  return valid;
}
...
function assertMaybeEnum(opts, name, a) {
  ...
  if (!valid)
    l.warn(
      ...
      'The `' +
        name +
        '` option will be ignored, because it is not one of the following allowed values: ' +
        a
          .map(function(x) {
...
function assertMaybeString(opts, name) {
  var valid =
    opts[name] === undefined ||
    (typeof opts[name] === 'string' && (0, _trim2.default)(opts[name]).length > 0);
  if (!valid)
    l.warn(
      opts,
      'The `' + name + '` option will be ignored, because it is not a non-empty string.'
    );
  return valid;
}
...
function assertMaybeArray(opts, name) {
  ...
  if (!valid)
    l.warn(opts, 'The `' + name + '` option will be ignored, because it is not an array.');
  return valid;
}
  ...
  if (!assertMaybeArray(opts, 'additionalSignUpFields')) {
    ...
    additionalSignUpFields = additionalSignUpFields.reduce(function(r, x) {
      ...
      if (type != undefined && (typeof type != 'string' || types.indexOf(type) === -1)) {
        l.warn(
          opts,
          'When provided, the `type` property of an element of `additionalSignUpFields` must be one of the following strings: "' +
            types.join('", "') +
            '".'
        );
        type = undefined;
...
function databaseUsernameStyle(m) {
  ...
  return l.hasSomeConnections(m, 'enterprise') && (0, _enterprise.findADConnectionWithoutDomain)(m)
    ? 'username'
    : 'email';
}
...
function resolveAdditionalSignUpSelectField(m, x) {
  ...
  var register = function register(m) {
    return resolvedPrefill !== undefined && resolvedOptions !== undefined
      ? (0, _index2.registerOptionField)(
          m,
          name,
          _immutable2.default.fromJS(resolvedOptions),
          resolvedPrefill
        )
      : m;
  };
  ...
  if (resolvedPrefill !== undefined && resolvedOptions !== undefined) {
    m = (0, _index2.registerOptionField)(
      m,
      name,
      _immutable2.default.fromJS(resolvedOptions),
      resolvedPrefill
